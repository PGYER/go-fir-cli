...
func (l *LarkNotifier) BuildAppPubishedMessage(apiAppInfo *api.ApiAppInfo, DownloadUrl string) string {
	title := l.CustomTitle
	if title == "" {
	...
	partialJSON := fmt.Sprintf(`
		...
		}`, title, apiAppInfo.Name, apiAppInfo.Type, time.Now(), DownloadUrl, l.CustomMsg)
